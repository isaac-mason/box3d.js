...
const filter = ( categoryBits: bigint, maskBits: bigint ) => ( { categoryBits, maskBits, groupIndex: 0 } );

function staticBox( x: number, y: number, z: number, hx: number, hy: number, hz: number, category: bigint, mask: bigint ): void
{
...
for ( let j = 0; j < 4; j++ )
{
	for ( let i = 0; i < num; i++ )
	{
		for ( let k = 0; k < num; k++ )
		{
			...
			def.position = { x: i * shift - centerx, y: j * shift + centery, z: k * shift - centerz };
			const body = b3.b3CreateBody( world, def );
			const shapeDef = b3.b3DefaultShapeDef();
			shapeDef.baseMaterial.friction = 1;
			shapeDef.filter = isGroupA ? filter( CUBE_A, GROUND | PLAT_A ) : filter( CUBE_B, GROUND | PLAT_B );
