...
// meshes
const matA = new THREE.MeshStandardMaterial( { color: 0x4d96ff, roughness: 0.5, transparent: true, opacity: 0.85 } );
const meshA = new THREE.Mesh( new THREE.BoxGeometry( 3, 3, 3 ), matA );
app.scene.add( meshA );
const matB = new THREE.MeshStandardMaterial( { color: 0xffd93d, roughness: 0.5, transparent: true, opacity: 0.85 } );
...
app.scene.add( line );
const markerA = new THREE.Mesh( new THREE.SphereGeometry( 0.12 ), new THREE.MeshBasicMaterial( { color: 0xffffff } ) );
const markerB = new THREE.Mesh( new THREE.SphereGeometry( 0.12 ), new THREE.MeshBasicMaterial( { color: 0xffffff } ) );
...
app.onFrame( ( dt ) =>
{
	...
	const q = new THREE.Quaternion().setFromEuler( new THREE.Euler( time * 0.8, time * 1.1, time * 0.5 ) );
	...
	linePos[0] = out.pointA.x; linePos[1] = out.pointA.y; linePos[2] = out.pointA.z;
	linePos[3] = out.pointB.x; linePos[4] = out.pointB.y; linePos[5] = out.pointB.z;
