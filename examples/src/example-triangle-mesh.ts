...
const SIZE = 44;
const height = ( x: number, z: number ) => Math.sin( x * 0.3 ) * 1.6 + Math.cos( z * 0.35 ) * 1.6 + Math.sin( ( x + z ) * 0.15 ) * 1.2;
...
b3.b3CreateMeshShape( terrainBody, b3.b3DefaultShapeDef(), meshData, { x: 1, y: 1, z: 1 } );
...
const terrainMesh = new THREE.Mesh( terrainGeom, new THREE.MeshStandardMaterial( { color: 0x3d6b4f, roughness: 0.95, flatShading: false } ) );
...
for ( let i = 0; i < 50; i++ )
{
	...
	b3.b3CreateSphereShape( body, shapeDef, { center: { x: 0, y: 0, z: 0 }, radius: rand( 0.5, 0.9 ) } );
}
