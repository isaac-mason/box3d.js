...
worldDef.workerCount = isolated ? Math.max( 2, Math.min( maxWorkers, hardware - 1 ) ) : 1;
...
function spawnCube(): void
{
	...
	def.position = { x: randomInRange( -SPAWN_AREA, SPAWN_AREA ), y: randomInRange( 0, SPAWN_HEIGHT ), z: randomInRange( -SPAWN_AREA, SPAWN_AREA ) };
...
app.onFrame( () =>
{
	...
	for ( let i = 0; i < 3 && cubes.length > 0; i++ )
	{
		...
		b3.b3Body_SetTransform( cube, { x: 0, y: randomInRange( 4, SPAWN_HEIGHT ), z: 0 }, { v: { x: 0, y: 0, z: 0 }, s: 1 } );
		b3.b3Body_SetLinearVelocity( cube, { x: randomInRange( -3, 3 ), y: 0, z: randomInRange( -3, 3 ) } );
	}
