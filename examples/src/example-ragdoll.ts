...
{
	for ( const h of humans ) { for ( const j of h.joints ) b3.b3DestroyJoint( j, false ); for ( const b of h.bodies ) b3.b3DestroyBody( b ); }
	humans = [];
	seed = 12345;
	for ( let i = 0; i < params.count; i++ )
	{
		const x = ( i - ( params.count - 1 ) / 2 ) * 1.6;
		const human = createHuman( b3, world, { x, y: 2.2 + rand() * 1.5, z: 0 }, group++, params.jointFriction );
		// give the torso a random tumble
		const m = 8;
		b3.b3Body_ApplyAngularImpulse( human.bodies[1], { x: ( rand() * 2 - 1 ) * m, y: ( rand() * 2 - 1 ) * m, z: ( rand() * 2 - 1 ) * m }, true );
		humans.push( human );
	}
}
respawn();

app.gui.add( params, 'count', 1, 10, 1 ).name( 'ragdolls' ).onChange( () => respawn() );
app.gui.add( params, 'jointFriction', 0, 0.3, 0.01 ).name( 'joint friction' ).onChange( () => respawn() );
app.gui.add( params, 'respawn' ).name( 'respawn (R)' );
window.addEventListener( 'keydown', ( e ) => { if ( e.key === 'r' || e.key === 'R' ) respawn(); } );
