...
const scenes: Array<[string, Scene]> = [
	['Revolute', ( ox, oz ) =>
	{
		...
		jd.base.bodyIdA = hub; jd.base.bodyIdB = arm;
		jd.base.localFrameA = { p: { x: 0, y: 5, z: 0 }, q: Z_TO_Y };
		jd.base.localFrameB = { p: { x: -2, y: 0, z: 0 }, q: Z_TO_Y };
		jd.enableMotor = true; jd.motorSpeed = 2; jd.maxMotorTorque = 20000;
	...
	['Prismatic', ( ox, oz ) =>
	{
		...
		jd.base.bodyIdA = g; jd.base.bodyIdB = slider;
		jd.base.localFrameA = { p: { x: 0, y: 5, z: 0 }, q: ID };
		jd.base.localFrameB = { p: { x: 0, y: 0, z: 0 }, q: ID };
		jd.enableLimit = true; jd.lowerTranslation = -2.5; jd.upperTranslation = 2.5;
		jd.enableMotor = true; jd.maxMotorForce = 50000;
		const joint = b3.b3CreatePrismaticJoint( world, jd );
		let t = 0;
		return ( dt ) => { t += dt; b3.b3PrismaticJoint_SetMotorSpeed( joint, Math.sin( t ) * 4 ); };
	}],
	['Spherical', ( ox, oz ) =>
	{
		const hub = ground( ox, oz );
		const bob = dyn( ox, oz, 0, 3, 0 );
		b3.b3CreateSphereShape( bob, b3.b3DefaultShapeDef(), { center: { x: 0, y: 0, z: 0 }, radius: 0.5 } );
		const jd = b3.b3DefaultSphericalJointDef();
		jd.base.bodyIdA = hub; jd.base.bodyIdB = bob;
		jd.base.localFrameA = { p: { x: 0, y: 5, z: 0 }, q: Z_TO_Y };
		jd.base.localFrameB = { p: { x: 0, y: 2, z: 0 }, q: Z_TO_Y };
		jd.enableConeLimit = true; jd.coneAngle = 0.6;
	...
	['Weld', ( ox, oz ) =>
	{
		...
		rj.base.bodyIdA = hub; rj.base.bodyIdB = a;
		...
		wj.base.bodyIdA = a; wj.base.bodyIdB = b;
	...
	['Distance', ( ox, oz ) =>
	{
		...
		jd.base.localFrameB = { p: { x: 0, y: 0, z: 0 }, q: ID };
		jd.length = 3; jd.enableSpring = true; jd.hertz = 1.5; jd.dampingRatio = 0.2;
	...
	['Wheel', ( ox, oz ) =>
	{
		...
		b3.b3CreateCapsuleShape( wheel, b3.b3DefaultShapeDef(), { center1: { x: -0.15, y: 0, z: 0 }, center2: { x: 0.15, y: 0, z: 0 }, radius: 1 } );
		const jd = b3.b3DefaultWheelJointDef();
		jd.base.bodyIdA = hub; jd.base.bodyIdB = wheel;
		jd.base.localFrameA = { p: { x: 0, y: 5, z: 0 }, q: Z_TO_Y };
		jd.base.localFrameB = { p: { x: 0, y: 0, z: 0 }, q: Z_TO_Y };
		jd.enableSuspensionSpring = true; jd.suspensionHertz = 2; jd.suspensionDampingRatio = 0.3;
		jd.enableSpinMotor = true; jd.spinSpeed = 6; jd.maxSpinTorque = 50000;
		b3.b3CreateWheelJoint( world, jd );
	}],
	['Motor', ( ox, oz ) =>
	{
		const target = b3.b3CreateBody( world, Object.assign( b3.b3DefaultBodyDef(), { type: b3.b3BodyType.b3_kinematicBody, position: { x: ox, y: 4, z: oz } } ) );
		const box = dyn( ox, oz, 3, 4, 0 );
		b3.b3CreateBoxShape( box, b3.b3DefaultShapeDef(), 0.5, 0.5, 0.5 );
		const jd = b3.b3DefaultMotorJointDef();
		jd.base.bodyIdA = target; jd.base.bodyIdB = box;
		jd.linearHertz = 4; jd.linearDampingRatio = 0.7; jd.maxSpringForce = 1e6;
		jd.angularHertz = 4; jd.angularDampingRatio = 0.7; jd.maxSpringTorque = 1e6;
		b3.b3CreateMotorJoint( world, jd );
		let t = 0;
		return ( dt ) => { t += dt; b3.b3Body_SetTransform( target, { x: ox + Math.cos( t ) * 2.5, y: 4 + Math.sin( t * 1.3 ) * 1.5, z: oz }, ID ); };
	}],
	['Parallel', ( ox, oz ) =>
	{
		const g = ground( ox, oz );
		...
		pin.base.bodyIdA = g; pin.base.bodyIdB = plate;
		...
		jd.base.bodyIdA = g; jd.base.bodyIdB = plate;
		jd.base.localFrameA = { p: { x: 0, y: 4, z: 0 }, q: Z_TO_Y };
		jd.base.localFrameB = { p: { x: 0, y: 0, z: 0 }, q: Z_TO_Y };
		jd.hertz = 2; jd.dampingRatio = 0.5; jd.maxTorque = 1e6;
	...
	['Filter', ( ox, oz ) =>
	{
		...
		jd.base.bodyIdA = a; jd.base.bodyIdB = b;
